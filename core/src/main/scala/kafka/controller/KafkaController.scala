...
class KafkaController(val config: KafkaConfig,
  ...
  /**
   * 不是假设当前broker就是controller节点，而是施加了一个session过期的监听器，并开始leader选举
   *
  ...
  def startup() = {
    zkClient.registerStateChangeHandler(new StateChangeHandler {
      ...
      // 发布注册并进行重新选举事件 RegisterBrokerAndReelect
      override def afterInitializingSession(): Unit = {
        eventManager.put(RegisterBrokerAndReelect)
      }
      // 在初始化session之前，先发布Expire事件，做一些清除工作
      override def beforeInitializingSession(): Unit = {
        val queuedEvent = eventManager.clearAndPut(Expire)

        // 等待Expire事件被处理完，内部使用CountDownLatch实现等待
    ...
    // 启动线程，不停的取队列中的事件交给KafkaController处理
    eventManager.start()
  }
  ...
  private def onControllerFailover(): Unit = {
    ...
    // 将这些changeHandler注册到zkClient的map中
    childChangeHandlers.foreach(zkClient.registerZNodeChildChangeHandler)

    // 将这些changeHandler注册到zkClient的map中，并注册对相应path注册zk的watcher
    ...
    // 初始化 ControllerContext 上下文
    info("Initializing controller context")
    initializeControllerContext()
    info("Fetching topic deletions in progress")
    val (topicsToBeDeleted, topicsIneligibleForDeletion) = fetchTopicDeletionsInProgress()
    // 初始化需要删除的topic和不能删除的topic列表，保存到controller上下文中
    info("Initializing topic deletion manager")
    topicDeletionManager.init(topicsToBeDeleted, topicsIneligibleForDeletion)

    // 在状态机启动之前，我们需要先发送 UpdateMetadataRequest 给所有其他的broker，因为只有这样，他们才能处理状态机发送的LeaderAndIsrRequests请求
    // We need to send UpdateMetadataRequest after the controller context is initialized and before the state machines
    // are started. The is because brokers need to receive the list of live brokers from UpdateMetadataRequest before
    // they can process the LeaderAndIsrRequests that are generated by replicaStateMachine.startup() and
    // partitionStateMachine.startup().

    // 发送更新元数据请求，这里主要是发送所有broker列表集合给所有的其他节点
    ...
    // 重新执行""/admin/reassign_partitions"下的副本迁移任务
  ...
  private def initializeControllerContext(): Unit = {
    ...
    // 对所有/brokers/topics/{topic}注册PartitionModificationsHandler到zkClient的map中
    ...
    replicaAssignmentAndTopicIds.foreach { case TopicIdReplicaAssignment(_, _, assignments) =>
      assignments.foreach { case (topicPartition, replicaAssignment) =>
        ...
        // 如果某个副本分配方案正在被迁移当中，那么将该topicPartition加入到被迁移列表partitionsBeingReassigned中
    ...
    // 对集群中所有broker，注册"/brokers/ids/{id}" 注册BrokerModificationsHandler，并在zk中增加watcher
    registerBrokerModificationsHandler(controllerContext.liveOrShuttingDownBrokerIds)
    // update the leader and isr cache for all existing partitions from Zookeeper
    // 遍历集群所有topicPartition，从zk的"/brokers/topics/{topic}/partitions/{partition}/state"下获取 LeaderIsrAndControllerEpoch 信息，并保存到controllerContext中
    updateLeaderAndIsrCache()
    // start the channel manager
    // 启动连接管理器，会与所有其他的broker建立网络socket连接
  ...
  private def fetchTopicDeletionsInProgress(): (Set[String], Set[String]) = {
    ...
    val topicsWithOfflineReplicas = controllerContext.allTopics.filter { topic => {
      // 该topic所有的副本 （topic+partition+replica）
      val replicasForTopic = controllerContext.replicasForTopic(topic)
      // 副本不在线，（broker宕机或者broker的磁盘满了）
    ...
    // 取并集 => 得到不能被删除的topic（副本所在broker不在线，或者是正在被重分配）
  ...
  private def updateLeaderAndIsrCache(partitions: Seq[TopicPartition] = controllerContext.allPartitions.toSeq): Unit = {
    // 获取 /brokers/topics/${topic}/partitions/${partition}/state 下的数据，保存的是分区的leader isr信息
  ...
  private def elect(): Unit = {
     ...
     */
    // 如果已经存在存在controllerId,说明当前broker不是controller节点
    ...
    try {
      // 升级controller的epoch "/controller_epoch" 由15 => 16
      // 注册"/controller" {"version":1,"brokerid":0,"timestamp":"1644315886451"}
  ...
  private def maybeBuildReassignment(topicPartition: TopicPartition,
    ...
    // 如果addingReplicas或者removingReplicas不为空，说明正在迁移进行中
    if (replicaAssignment.isBeingReassigned) {
  ...
  // 实现ControllerEventProcessor，根据不同事件类型，进行响应的处理，这其实是命令模式
